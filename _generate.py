...
cat("actor", "Actors (ACTOR)",
 ...
 [
  ...
  K("CANDELABER","(flag)","both","Only effective if GROUND is not set: the object attaches to the ceiling of its region instead of the floor. Useful for pendant objects like stalactites or candelabra."),
...
# ---- Additional v3.8 / v3.9 keywords ------------------------------------
# Curated from the ACKNEX v3.8 (3dmanual.pdf) and v3.9 (ACKMAN.PDF) manuals.
# These extend the categories defined above (order within a group does not
# matter; keywords are sorted alphabetically at generation time).
EXTRA = {
 "texture": [
  K("ALBEDO","Number;","new","Reflection factor of the texture (0..1, default 0) for light from LIGHT_ANGLE or a separate light source; gives buildings, columns and ledges a more three-dimensional look."),
  K("RADIANCE","Number;","new","Maximum brightness of the texture (0..1, default 0); with high ambient light, RADIANCE>0 textures are tinted with the palette's brightness colour."),
  K("AUTORANGE","(flag)","new","Automatically shades every palette colour contained in any RANGE — even unsorted ones — according to their similarity to the INFINITY colour."),
  K("MAPCOLOR","Number;","new","Colour number used to draw a MODEL texture as a non-textured wireframe (WIRE) and on the automap."),
  K("CLUSTER","(flag)","new","MODEL textures only: displays just the vertices of the model's polygons."),
  K("LIGHTMAP","(flag)","new","ATTACH-texture flag that maps pre-computed light and shadow onto a wall texture; the LIGHTMAP bitmap must contain only shading-range colours."),
  K("SCALE_X","x;","new","Horizontal scaling of the texture in pixels per step (for floor/ceiling textures, scaling along the X axis)."),
  K("SCALE_Y","y;","new","Vertical scaling of the texture in pixels per step (for floor/ceiling textures, scaling along the Y axis)."),
  K("ATTACH","texture;","new","Attaches an additional, transparent texture to a wall, thing or actor texture — for paintings, inscriptions, bullet holes or shadows. Walls allow a chain of ATTACH textures; things/actors allow only one (set on the object, not its texture)."),
  K("ATTACHED","(flag)","new","Set automatically while the object's texture carries an ATTACH texture."),
  K("SHADOW","(flag)","new","The ATTACH texture is drawn at the actor's floor height; with a dark DIAPHANOUS texture this produces an actor shadow."),
  K("TOUCH","string;","new","Text string shown next to the mouse pointer when it touches the wall/thing/actor carrying this texture (requires TOUCH_MODE >= 1)."),
  K("TOUCH_RANGE","Number;","new","Maximum distance (in steps) at which the object still reacts to the mouse for TOUCH and texture mouse events; IMMATERIAL objects are invisible to the mouse."),
 ],
 "wall": [
  K("IMPASSABLE","(flag)","new","Wall is impenetrable over its entire length and height even when invisible; combined with PASSABLE it stays passable for the player but not for actors."),
 ],
 "region": [
  K("FLOOR_ANGLE","Number;","new","Rotation angle of the floor texture (0..2pi, default 0), one full value being a complete revolution around the region's GENIUS centre."),
  K("CEIL_ANGLE","Number;","new","Rotation angle of the ceiling texture (0..2pi, default 0) around the region's GENIUS centre."),
  K("FLOOR_ASCEND","(flag)","new","Lifts the floor corner points upward by the Z values of the region's vertices, producing a sloped floor."),
  K("FLOOR_DESCEND","(flag)","new","Lowers the floor corner points by the Z values of the region's vertices, producing a sloped floor."),
  K("CEIL_ASCEND","(flag)","new","Lifts the ceiling corner points upward by the Z values of the region's vertices, producing a sloped ceiling."),
  K("CEIL_DESCEND","(flag)","new","Lowers the ceiling corner points by the Z values of the region's vertices, producing a sloped ceiling."),
  K("SAVE_ALL","(flag)","new","All walls and things inside the region are stored by SAVE instructions — useful when the region is moved at runtime (e.g. by ROTATE)."),
 ],
 "actor": [
  K("ASPEED","Number;","new","Maximum rotation speed of the actor in radians per tick (default 0 = disabled); makes multi-sided or MODEL actors turn more naturally along their WAYs."),
  K("REL_ANGLE","Number;","new","Angle of a TARGET STICK actor relative to the player; can be changed to move the actor around the player."),
  K("REL_DIST","Number;","new","Distance of a TARGET STICK actor from the player; can be changed to move the actor around the player."),
  K("TARGET_X","Number;","new","Target X position of the actor (e.g. the on-screen position of a HOLD actor)."),
  K("TARGET_Y","Number;","new","Target Y position of the actor (e.g. the on-screen position of a HOLD actor)."),
  K("NO_CLIP","(flag)","new","Thing/actor texture is not cut off at the floor or ceiling of a region."),
  K("CLIP","(flag)","new","Thing/actor texture is cut off at the floor or ceiling of a region, preventing objects from sticking through."),
  K("MOVED","(flag)","new","Set automatically right after the actor has moved; used with CAREFULLY/EACH_TICK and the ACTOR_IMPACT/ACTOR_FLOOR_HGT skills."),
 ],
 "files": [
  K("MODEL","Name, <file.MDL>;","new","Assigns a textured 3-D model file (.MDL format) to a name for use as an actor/thing texture."),
 ],
 "globals": [
  K("ACTOR_DIST","(skill)","new","Ratio of an actor's CLIP_DIST to the wall CLIP_DIST; must be <= THING_DIST."),
  K("THING_DIST","(skill)","new","Ratio of a thing's CLIP_DIST to the wall CLIP_DIST (0..1, default 1)."),
  K("ACTOR_CLIMB","(skill)","new","Maximum step height (default 1) actors can climb during TARGET movements, like PLAYER_CLIMB."),
  K("ACTOR_WIDTH","(skill)","new","Factor (default 1) the texture width of all actors is multiplied by for collision detection."),
  K("THING_WIDTH","(skill)","new","Factor (default 1) the texture width of all things is multiplied by for collision detection."),
  K("SKIP_FRAMES","(skill)","new","Number of frames (default 5) that invisible actors beyond their DIST skip between movements, to save rendering time."),
  K("ACTOR_IMPACT_VX","(skill)","new","Actor's rebound speed in X on hitting an obstacle; valid only with CAREFULLY set, inside EACH_TICK, right after a move (MOVED set)."),
  K("ACTOR_IMPACT_VY","(skill)","new","Actor's rebound speed in Y on hitting an obstacle; valid only with CAREFULLY set, inside EACH_TICK, right after a move (MOVED set)."),
  K("ACTOR_IMPACT_VZ","(skill)","new","Actor's rebound speed in Z on hitting floor/ceiling; valid only with CAREFULLY set, inside EACH_TICK, right after a move (MOVED set)."),
  K("ACTOR_FLOOR_HGT","(skill)","new","Floor height at the actor's position; valid only for TARGET-moved actors with CAREFULLY, inside EACH_TICK, right after a move."),
  K("ACTOR_CEIL_HGT","(skill)","new","Ceiling height at the actor's position; valid only for TARGET-moved actors with CAREFULLY, inside EACH_TICK, right after a move."),
  K("MAP_OFFSX","(skill)","new","Horizontal offset (pixels, default 0) of the automap centre from the 3-D window centre."),
  K("MAP_OFFSY","(skill)","new","Vertical offset (pixels, default 0) of the automap centre from the 3-D window centre."),
  K("MAP_CENTERX","(skill)","new","Horizontal offset of the automap centre in steps (independent of MAP_SCALE), keeping the centre fixed."),
  K("MAP_CENTERY","(skill)","new","Vertical offset of the automap centre in steps (independent of MAP_SCALE), keeping the centre fixed."),
  K("MAP_EDGE_X1","(skill)","new","Left limit of the automap display, in pixels from the screen's upper-left corner (default = screen size)."),
  K("MAP_EDGE_X2","(skill)","new","Right limit of the automap display, in pixels from the screen's upper-left corner."),
  K("MAP_EDGE_Y1","(skill)","new","Top limit of the automap display, in pixels from the screen's upper-left corner."),
  K("MAP_EDGE_Y2","(skill)","new","Bottom limit of the automap display, in pixels from the screen's upper-left corner."),
  K("MAP_ROT","(skill)","new","When 1, the automap rotates with the player angle like a radar display; only objects within CLIP_DIST are shown."),
  K("COLOR_PLAYER","(skill)","new","Colour number of the player symbol on the automap (default 7)."),
  K("COLOR_ACTORS","(skill)","new","Default colour number of actor symbols on the automap (default 3)."),
  K("COLOR_THINGS","(skill)","new","Default colour number of thing symbols on the automap (default 13)."),
  K("COLOR_WALLS","(skill)","new","Default colour of walls on the automap (default 244); a shading-range colour gives antialiased walls."),
  K("COLOR_BORDER","(skill)","new","Default colour of border walls on the automap (default 244); a shading-range colour gives antialiased borders."),
  K("TOUCH_MODE","(skill)","new","When 1 (default), the TOUCH texture string of a touched object is shown at the mouse pointer."),
  K("TOUCH_DIST","(skill)","new","Maximum distance (default 100 steps) for triggering the texture mouse events TOUCH, IF_TOUCH, IF_RELEASE and IF_CLICK."),
  K("TOUCH_STATE","(skill)","new","0 normally; 1 when the pointer is on a TOUCH-text object, 2 when on an IF_CLICK object, 3 when both."),
  K("MOUSE_CALM","(skill)","new","Maximum movement still treated as immobility for MOUSE_MOVING (default 3)."),
  K("MOUSE_TIME","(skill)","new","Time in ticks over which MOUSE_CALM is measured to determine MOUSE_MOVING (default 4)."),
  K("MOUSE_MOVING","(skill)","new","1 while the mouse is moving, 0 when it has been stationary for a quarter second."),
  K("MICKEY_X","(skill)","new","Mouse movement in dots along X since the last frame; can drive MOUSE_X."),
  K("MICKEY_Y","(skill)","new","Mouse movement in dots along Y since the last frame; can drive MOUSE_Y."),
  K("MOUSE_X","(skill)","new","Horizontal mouse-pointer position in pixels from the screen's upper-left corner; settable when the pointer is on."),
  K("MOUSE_Y","(skill)","new","Vertical mouse-pointer position in pixels from the screen's upper-left corner; settable when the pointer is on."),
  K("MOUSE_ANGLE","(skill)","new","Direction angle of the mouse pointer in the level (e.g. for DROP); computed from MOUSE_X and PLAYER_ANGLE, may be set directly."),
  K("JOYSTICK_X","(skill)","new","Joystick X-axis movement in the range -255..+255."),
  K("JOYSTICK_Y","(skill)","new","Joystick Y-axis movement in the range -255..+255."),
  K("STR_LEN","(skill)","new","Number of characters of the STRING last entered through INKEY."),
  K("CDAUDIO_VOL","(skill)","new","Volume of the currently playing CD audio track (0..1); under plain DOS only."),
  K("CHANNEL","(skill)","new","Number (0..7) of the sound channel the last sound was assigned to, or -1 if it could not be played."),
  K("CHANNEL_0...CHANNEL_7","(skills)","new","State of each of the 8 sound channels: 0 free, 1 normal sound playing, 2 SLOOP sound playing."),
  K("BLUR_MODE","(skill)","new","When 1, motion blur is always on; at 0.5 it activates whenever the player or mouse moves (default 0)."),
  K("EYE_DIST","(skill)","new","Eye separation for 3-D glasses (default 0.5 steps; requires -3D_SIMEYE); 0 disables the stereoscopic effect."),
  K("LOAD_MODE","(skill)","new","When 1, a LOAD or LEVEL instruction does not change the screen resolution (default 0)."),
  K("SVDIST","(skill)","new","Critical vertical range (steps) within which a thing/actor sound is audible (default 0 = inactive)."),
  K("MAX_DIST","(skill)","new","Distance of the farthest visible vertex within CLIP_DIST; useful for tuning region CLIP_DISTs."),
  K("PLAYER_DEPTH","(skill)","new","Depth of an underwater BELOW region beneath the player, its ceiling matching the player region's floor."),
  K("IMPACT_VROT","(skill)","new","Change of angle of a TARGET STICK actor relative to the player after bouncing off an obstacle; can spin the player."),
  K("BOUNCE_VX","(skill)","new","Reflection vector in X when the player hits a wall; add it to the player speed for a pinball-like bounce."),
  K("BOUNCE_VY","(skill)","new","Reflection vector in Y when the player hits a wall; add it to the player speed for a pinball-like bounce."),
  K("SLOPE_X","(skill)","new","Ascent of the region floor along the X axis (rise per step)."),
  K("SLOPE_Y","(skill)","new","Ascent of the region floor along the Y axis (rise per step)."),
  K("SLOPE_SIDE","(skill)","new","Ascent of the region floor perpendicular to the player's line of sight (rise per step)."),
  K("MOVE_ANGLE","(skill)","new","Actual movement direction of the player."),
  K("DELTA_ANGLE","(skill)","new","Angular difference between MOVE_ANGLE and PLAYER_ANGLE."),
  K("KEY_ANY","(skill)","new","1 while any key is pressed, otherwise 0."),
  K("KEY_A...KEY_Z","(skills)","new","1 while the corresponding letter key is pressed, otherwise 0."),
  K("KEY_F1...KEY_F12","(skills)","new","1 while the corresponding function key is pressed, otherwise 0."),
  K("FLIC_FRAME","(skill)","new","Frame number of the currently playing flic; useful for syncing sound/music from a global EACH_TICK action."),
  K("LIGHT_ANGLE","(skill)","new","Direction (0..2pi) of the infinitely distant light source; ALBEDO textures reflect light from this direction."),
  K("SKY_OFFS_X","(skill)","new","Horizontal shift of all sky textures in pixels (never negative); periodically increasing it gives a wandering-cloud effect."),
  K("SKY_OFFS_Y","(skill)","new","Vertical shift of all sky textures in pixels."),
 ],
 "actions": [
  K("ABS","ABS(x)","new","Absolute value of x, for use in RULE expressions."),
  K("INT","INT(x)","new","Integer part of x."),
  K("EXP","EXP(x)","new","e raised to the power x."),
  K("SIGN","SIGN(x)","new","-1 if x<0, 1 if x>0, 0 if x==0."),
  K("SQRT","SQRT(x)","new","Square root of x (a negative argument aborts the engine)."),
  K("SIN","SIN(x)","new","Sine of x (radians)."),
  K("COS","COS(x)","new","Cosine of x (radians)."),
  K("TAN","TAN(x)","new","Tangent of x (radians)."),
  K("ASIN","ASIN(x)","new","Arc sine of x (inverse trigonometric)."),
  K("ACOS","ACOS(x)","new","Arc cosine of x (inverse trigonometric)."),
  K("LOG","LOG(x)","new","Natural logarithm of x."),
  K("LOG10","LOG10(x)","new","Base-10 logarithm of x."),
  K("LOG2","LOG2(x)","new","Base-2 logarithm of x."),
  K("IF","(expr) { ... } ELSE { ... }","new","Executes the first block when the expression is non-zero, otherwise the optional ELSE block."),
  K("ELSE","{ ... }","new","Optional alternative block of an IF, run when the condition is false."),
  K("WHILE","(expr) { ... }","new","Repeats the block as long as the expression is non-zero."),
  K("BREAK",";","new","Leaves the enclosing WHILE loop immediately."),
  K("CONTINUE",";","new","Skips to the next iteration of the enclosing WHILE loop."),
  K("ENDIF",";","new","Closes an IFDEF/IFNDEF preprocessor block; the lines in between are skipped when the condition is not met."),
  K("SET_ALL","[Obj1.]Keyword1, [Obj2.]Keyword2;","new","Like SET, but applies to all objects/regions matching the keyword or synonym, not just the first."),
  K("SET_STRING","String1, String2;","new","Copies String2 into String1 (without exceeding String1's original length)."),
  K("ADD_STRING","String1, String2;","new","Appends String2 to String1, like SET_STRING but concatenating."),
  K("TO_STRING","String, Skill;","new","Writes the numeric value of Skill as digits into String."),
  K("SET_SKILL","Skill, String;","new","Sets Skill to the numeric value contained in String."),
  K("SET_INFO","String, Object;","new","Built-in debugger: writes all interesting parameters of Object into String for on-screen display."),
  K("PRINTFILE","Name, Number/Skill;","new","Names a text file (5-char name plus 3-digit number) into which PRINT_VALUE and PRINT_STRING write."),
  K("PRINT_VALUE","Skill;","new","Appends Skill as a number with 3 decimals to the PRINTFILE text file."),
  K("PRINT_STRING","String;","new","Appends a string (or quoted text) to the PRINTFILE text file; \\n starts a new line."),
  K("INKEY","String;","new","Reads keyboard input into the named string until input is terminated; activates the local keyboard layout."),
  K("TILT","Region, Number/Skill;","new","Multiplies the Z values of all vertices, things and actors in the region, changing its slope at runtime."),
  K("SAVE_DEMO","Name, Number/Skill;","new","Records the movement of the player and actors and all game events to a demo file."),
  K("PLAY_DEMO","Name, Number/Skill;","new","Replays a recorded demo file; pressing any key stops playback."),
  K("STOP_DEMO",";","new","Stops recording or replaying the demo file."),
  K("PLAY_SOUNDFILE","Name;","new","Plays a digital sound effect directly from an external sound file."),
  K("INPORT","Skill, adr;","new","Reads the byte at hardware port adr into Skill."),
  K("OUTPORT","Number/Skill, adr;","new","Writes a number or Skill to hardware port adr."),
  K("GETMIDI","Channel, Status;","new","Gets the device type of a MIDI channel (0..15)."),
  K("SETMIDI","Channel, Status;","new","Sets the device type of a MIDI channel (0..15): 0 = FM, 1 = digital, etc."),
  K("MIDI_COM","Statusb, data1, data2;","new","Sends data directly to a MIDI channel; statusb = command*16 + channel number."),
  K("NEXT_MY",";","new","Moves the MY synonym to the next object of the same name — for looping over several objects."),
  K("NEXT_THERE",";","new","Moves the THERE synonym to the next matching object — for looping over several objects."),
  K("NEXT_MY_THERE",";","new","Advances both MY and THERE synonyms to the next matching object — for looping over several objects."),
  K("IF_MSTOP","Action;","new","Triggered when the mouse pointer is active and held still for half a second (see MOUSE_CALM/MOUSE_MOVING)."),
  K("IF_LOAD","Action;","new","Performed after a saved score is loaded; useful for resetting panels or skills used while saving."),
  K("IF_TOUCH","Action;","new","Texture mouse event: performed while the mouse pointer touches the object's texture (within TOUCH_DIST)."),
  K("IF_RELEASE","Action;","new","Texture mouse event: performed when the mouse pointer leaves the object's texture."),
 ],
 "ui": [
  K("CENTER_X","(flag)","new","Centres a TEXT horizontally on POS_X (otherwise it is left-justified)."),
  K("CENTER_Y","(flag)","new","Centres a TEXT vertically on POS_Y (otherwise it is top-justified)."),
  K("RELPOS","(flag)","new","Panel position (POS_X, POS_Y) is relative to the 3-D window's upper-left corner instead of the screen's; such panels are clipped to the window."),
 ],
 "synonyms": [
  K("HIT","(synonym)","new","The object last hit by the player's SHOOT, last clicked with the mouse, or nearest to the last EXPLODE."),
  K("TOUCHED","(synonym)","new","The object last touched by the mouse pointer."),
  K("TOUCH_REG","(synonym)","new","The region whose floor or ceiling was last touched by the mouse pointer."),
  K("TOUCH_TEX","(synonym)","new","The texture last touched by the mouse pointer."),
 ],
}
for _c in CATS:
    if _c["slug"] in EXTRA:
        _c["kws"].extend(EXTRA[_c["slug"]])
